<template>
    <div class="shopbottom">
        ...
        <div class="shopbut">
            ...
            <a href="javascript:" class="buy" @click="buy" :style="applyStatus != '05'?'background:#ddd; ' : ''"><span>立即购买</span></a>
        </div>
    </div>
</template>
...
export default {
    props:['shop','cartList','isMyStore','ApplyStatus','iSeq','Wseq','wholesaleSeq'],
    data(){
        return{
           list:0,
           shopMai:this.shop,
    ...
    methods:{
        ...
        gohome(){
            let wholesaler = sessionStorage.getItem('Wseq');
            // 如果是新版区域批发商
            if (this.blockType) {
                this.$router.push({path:'/block/wholesaleIndex?from=areaWholesaler&wholesaler='+sessionStorage.getItem("Wseq")});
                return
            }
            //如果是分享进入的，则点击进入店铺只能进入区域批发商店铺
            if(this.shareIstrue){
                alert(999)
                this.$router.push({path:'/wholesale/business',query:{wholesaler:this.wholesaleSeq||this.Wseq,share:true,seq:this.iSeq}});
                return
            }

            if (this.isApplyThrough && this.userBlock) {// 区域批发商身份 进入店铺
                this.$router.push({path:'/wholesale/business',query:{wholesaler:this.iSeq || this.wholesaleSeq ||this.Wseq,seq:this.iSeq,from:'progress'}});
                return
            }
            this.$router.push({path:'/business',query:{wholesaler:this.wholesaleSeq ||this.Wseq}});
        },
        // 添加商品到购物车
        addCart(){

            if (this.isApplyThrough && this.userBlock) {
        ...
        listfn(){
            ...
            axios.get(this.URL + '/OrderNewBuycar/GetBuyCarCount?seq='+sessionStorage.seq).then(res => {
                this.list = res.data;
                Indicator.close();
                if(this.cartList > 0) Toast('已添加到购物车。');
            });
        }
    }
}
