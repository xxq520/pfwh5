          ...
          <div class="detailcontent" v-for="(pitem,index) in item.ordersdtl" :key="index">
            ...
            <div class="goodsname">
                <div class="line">
                    ...
                    <span class="price">￥<i>{{pitem.dlprice}}</i></span>
                </div>
                <div class="line">
...
export default {
    ...
    data() {
        return {
            ...
            unitName:'',
            reason:'',
            TatolPrice:this.$route.query.totalp,
            ProductImg:'',
            data:[],
        ...
        toreturndetails(){
            ...
            showToast('退款理由不能为空！')
                ...
                MessageBox({
                    ...
                    if (action == 'confirm') {
                        axios.post(this.URL + '/BusinessOrders/SetOrdersOnlyRefund',{seq:this.seq,salenumber:this.salenumber,reason:this.reason})
                        .then(res=>{
                            if (res.status == 200) {
                                if (res.data.code == 200) {
