  ...
  <div class="popup-bg">
    <div class="popup-box">
      ...
      <div class="popup-item">
        ...
        <div class="pop-text">
          <p>￥{{ queryType?listData.FactoryPrice:(listData.DisCountPrice || listData.ShopPrice) }}</p>
          ...
          <span v-if="!isMyStore">库存量：{{ listData.StockCount?listData.StockCount:0 }}</span>
          <span v-else>库存量：{{ listData.StockCount && listData.StockCount > 0?'有现货':'无现货'}}</span>
        </div>
      </div>
      <!--规格选择信息-->
      <div class="popup-item" v-if="postData" v-for="(item,index) in postData.proStandardJoints" :key="index">
      ...
      <!--购买数量-->
      <div class="popup-item" style="padding:0.4rem 0;" v-if="isShowBuy" v-show="isMyStore&&isType&&applyStatus=='05'">
        <div class="add-text">购买数量</div>
        <div class="add-box">
          <!-- <el-input-number v-model="num" @change="handleChange" :min="listData.Moq" :max="listData.StockCount" style="width:7rem;"></el-input-number> -->
          <el-input-number v-model="num<1?1:num" @change="handleChange" style="width:7rem;"></el-input-number>
        </div>
      </div>
    </div>
    <!--按钮-->
    <div class="popup-btn"
         v-if="isShowBuy"
         v-show="isMyStore&&isType/*&&applyStatus=='05'*/">
      <a href="javascript:" class="addCart" @click="addCart"><span>加入购物车</span></a>
      <a href="javascript:" class="buy" @click="buy" :style="{background:'#ddd'}"><span>立即购买</span></a>
    </div>
  </div>
</template>
<script>
  export default {
    props:["shift","detail","isType","isMyStore","ApplyStatus"],
    data(){
      return {
        num: 1,
        visible: this.shift,
    ...
    /*  this.isApplyThrough = this.$store.state.userInfo.ApplyStatus == 2;
      this.userBlock = this.$store.state.userInfo.userBlock;
        this.applyStatus = this.ApplyStatus
        if (this.isApplyThrough) {
          // this.applyStatus == "1000"
        }*/
    },
    mounted() {
      let that = this;
    ...
    methods:{
      ...
      buy(){
        ...
        axios.post(this.URL + '/Product/PurchasingCertainProduct',post).then(res => {
          ...
          sessionStorage.setItem('certificate_type',data.listOrder[0].Type);
      ...
      addCart(){
        if(this.listData.StockCount <= 0){
          ...
          return false;
        }else if(this.num == '' || this.num == ' '|| this.num == null|| this.num == undefined){
           Toast({
            message: '购买数量不能为空，请重新选择购买数量',
            className: 'counttc',
            duration: 2000
          });
          return false;
        ...
        that.postData.proStandardJoints.forEach((post,index) => {
          (function(p,i){
            for(var y=0;y<p.length;y++){
              if(obj['StandardName'+(i+1)] == p[y].Name){
                that.getData['StandardName'+(i+1)] = p[y].Name;
                return false;
              }
            }
