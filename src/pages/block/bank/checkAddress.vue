                ...
                <div class="Selection">
                    ...
                    <div class="box">
                        <ul class="provinceData" :class="{'isShow':areaType === 1}" type="1">
                            <template v-for="(province,index) in provinceData">
                                <li v-if="province.id!=1"
...
<script>
    ...
    export default {
        ...
        methods: {
            goBack(){
                this.$emit('bindtap', 1, this.selectLocation);
            },
            /**
             * 显示地区模态框
             * @param isShow
            ...
            handleSelect(type, item) {
                if (type !== null) {
                    ...
                    for (let key in this.selectLocation) {
                        if (Number(type) <= Number(key)) this.selectLocation[key] = '';
                    }
                } else {
                    //保存已选地区
            ...
            async getAreaData(type = 1, pid = 0) {
                const res = await get(this.blockURL+`/qypfs-user/userBankCard/shopregion?type=${type}&pid=${pid}`);
                if (res.code === 200) {
                    switch (type) {
                        case 1:
