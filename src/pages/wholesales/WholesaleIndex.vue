<template>
    <div class="glode">
        <header class="mint-header"
            v-if="(userType != 3&&userType !=4&&userType !=6&&userType !=7&&!shareIstrue)||userBlock">
        ...
        <div class="shopxx">
            ...
            <div class="orderStatus share" v-if="userBlock||shareIstrue" @click="changeShare"><img
                    src="../../assets/images/dh_icon_sharebtn.png" alt=""><span>分享</span></div>
            <span class="orderStatus" v-show="userType != 5 && userType != 6&&!userBlock&&!shareIstrue"
                @click="OrderState">平台订货</span>
        </div>
        <div v-if="(userType != 3&&userType !=4&&userType !=6&&userType !=7)||userBlock">
            <div class="topnav">
                <ul>
                    <li v-for="(item,index) in classify" :class="{lihover:classli==index}"
                        @click="lihover(index,item.Id)" :key="index">{{ item.Name }}<i></i></li>
                </ul>
            </div>

        </div>
        <div class="shopChange" v-if="(userType != 3&&userType !=4&&userType !=6&&userType !=7)||userBlock">
            <el-row>
                <el-col :span="12" @click.native.capture="synthesis">综合<i
                        :class="Sort==1?'el-icon-caret-top':'el-icon-caret-bottom'"></i></el-col>
                <el-col :span="12" @click.native.capture="pricefn">价格<i
        ...
        <div class="shopbox" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading"
            infinite-scroll-distance="80" v-if="(userType != 3&&userType !=4&&userType !=6&&userType !=7)||userBlock">
            <div class="shopmain">
                <!--list-->
                <div class="shoplist" v-for="(item,index) in product" :key="index" @click="linkTo(item.Code)">
                    ...
                    <div class="shoptext">
                        ...
                        <div class="shopdeal" v-if="sales">
                            <p style="margin-bottom:0.4rem"><i
                                    class="letter">已售{{ countSale(item.CountSale) }}{{item.Unit}}</i><i
                                    class="order">{{ countSale(item.Moq) }}{{item.Unit}}起订</i></p>
                            <p><span class="money">
                                    <!--<i>促</i>-->￥{{ item.Price | formatPrice }}</span></p>
                        </div>
                        <div class="shopdeal" v-else>

                            <p><span class="money">
                                    <!--<i>促</i>-->￥{{ item.Price | formatPrice }}</span><i
                                    class="order">{{ countSale(item.Moq) }}{{item.Unit}}起订</i></p>
                        </div>
                    </div>
                    <!--list end-->
        ...
        <wholesale-nav
            v-else-if="userType == 2||userType == 3||userType == 4||userType ==6||userType == 7||userType == 8||userBlock"
            :Wseq='seq'></wholesale-nav>
        ...
        <!-- 分享组件 -->
        <share ref="share" :shareData="share"></share>
    </div>
</template>
<script>
    ...
    export default {
        ...
        mounted() {
            let that = this;
            // console.log(this.shopData.shopname)
        ...
        methods: {
            ...
            isPassSettled() {
                if (this.status == 0) {
                    ...
                    if (!this.$route.query.from && this.$route.query.from != 'progress' && !this.userBlock && !obj
                        .share) {
                        this.$router.push({
                    ...
                    MessageBox({
                        ...
                        if (action == 'confirm') {
                            location.href = this.SETTLED + '?wholesaler=' + this.seq +
                                '&rewrite=true&isIndex=true';
                        } else {
                            this.loginOut();
                        }
                    })
            ...
            brandSqueryLink() {
                ...
                if (this.brandSquareStatus == 0 || this.brandSquareStatus == 4 || this.brandSquareStatus == 5 || this
                    ...
                    window.location.href = this.BRANDSQUAREPROGRESS + "?s=" + this.seq + "&status=" + this
                        .brandSquareStatus;
                }
            },
            // 退出登录
                ...
                axios.get(this.URL + '/WholesaleManage/GetWholesalerInfoDetail', {
                    ...
                    var shareObj = {
                        ...
                        url: location.origin + location.pathname +
                            `#/wholesale/business?wholesaler=${this.wholesaler}&share=true&seq=${this.seq||this.shareIstrue}`,
                    ...
                    if (res.data.MarketAuditStatus == 0 && res.data.FinanceAuditStatus == 0) { //批发商入驻/--市场待审
                        this.status = 0;
                    } else if (res.data.MarketAuditStatus == 1 && res.data.FinanceAuditStatus ==
                        0) { //批发商入驻/品牌商入驻--财务待审
                        this.status = 1;
                    } else if (res.data.MarketAuditStatus == 2 && res.data.FinanceAuditStatus ==
                        0) { //批发商入驻/品牌商入驻--市场驳回
                        this.status = 2;
                    } else if (res.data.MarketAuditStatus == 1 && res.data.FinanceAuditStatus ==
                        1) { //批发商入驻/品牌商入驻--财务通过
                    ...
                    // 品牌商入驻审核过程
                    if (res.data.MarketAuditStatus == 0 && res.data.FinanceAuditStatus == 0 && res.data
                        .HQAuditStatus ==
                        0) { //品牌商入驻/--市场待审
                    ...
                    if (((this.userType == 2 && this.status == 4) || this.userType == 8) && !res.data
                        .IsExpired) {
                        this.checkUpload();
                    }
                    // 校验品牌商入驻状态(暂停使用)
            ...
            goSettled() {
                if (this.status == '-1') {
                    this.hrefs = this.SETTLED + '?isServicer=true&usertype=' + this.userType + '&wholesaler=' + this
                        .wholesaler +
            ...
            OrderState() {
                ...
                if (fromApp) {
                    ...
                    let iosDownUrl =
                        'itms-apps://itunes.apple.com/WebObjects/MZStore.woa/wa/viewSoftware?id=1243425475';
                    this.configSystem()
